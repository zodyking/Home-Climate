...
class HomeWeatherPanel extends HTMLElement {
  ...
  connectedCallback() {
    this._render();
    this._loadConfig();
    this.shadowRoot.addEventListener("click", this._onPanelClick);
  }

  disconnectedCallback() {
    this._stopRefresh();
    if (this.shadowRoot) this.shadowRoot.removeEventListener("click", this._onPanelClick);
  }

  _onPanelClick = (e) => {
    if (e.target.closest("[data-action='toggle-settings']") || e.target.closest(".settings-btn")) {
      e.preventDefault();
